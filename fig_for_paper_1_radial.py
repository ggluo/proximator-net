...
l1_recon = utils.bart(1, 'pics -R W:3:0:%f -d5 -i100 -e -t'%(recon_config['bart_l1_lambda']), traj, radial_ksp, coilsen1[:,:,np.newaxis,:])
utils.save_img(abs(np.squeeze(l1_recon)), os.path.join(fig_path, 'l1_recon'), np.min(abs(l1_recon)), np.max(abs(l1_recon)))
...
for i in range(recon_config['sense_iter']):
    ...
    img_k = img_k*0.9 + 0.1*np.squeeze(utils.float2cplx(sess.run(op_prox, {xs: utils.cplx2float(img_k[np.newaxis, ...])})))
    
    img_k = utils.scale_up(img_k, scalar)
    utils.save_img(abs(img_k), fig_path+'/img_k_'+str(i), np.min(abs(img_k)), np.max(abs(img_k)))
...
for i in range(100):
    ...
    img_k = img_k*(1-la) + la*np.squeeze(utils.float2cplx(sess.run(op_prox, {xs: utils.cplx2float(img_k[np.newaxis, ...])})))
    img_k = utils.scale_up(img_k, scalar)
    psnr_net_radial = utils.psnr(abs(img_k)/norm(abs(img_k)), rss)
    ssim_net_radial = utils.ssim(abs(img_k)/norm(abs(img_k)), rss)
...
curve_s = np.array(curve_s)
curve_net = np.array(curve_net)
...
ax1.plot(np.arange(0,itera), curve_net[:,0], label='lamda=0.1')
ax1.plot(np.arange(0,itera), curve_s[:,0], label='lamda=0')
ax1.set_xlabel('iteration', fontsize=label_size)
ax1.set_ylabel('PSNR', fontsize=label_size)
ax1.grid(color='gray', linestyle='dashed')

ax2.plot(np.arange(0,itera), curve_net[:,1], label="lamda=0.1")
ax2.plot(np.arange(0,itera), curve_s[:,1], label='lamda=0')
